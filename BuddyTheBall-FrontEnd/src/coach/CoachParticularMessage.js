...
export default function CoachParticularMessage({ route }) {
    ...
    useEffect(() => {
        const getMessagesBySenderId = async () => {
            console.log("fghbtgf---->", state.authPage.auth_data._id, route.params.messages);
            const result = await GetMessagesBySenderIdReceiverId(state.authPage.auth_data._id, route.params.messages.receiver_id);
            if (result) {
